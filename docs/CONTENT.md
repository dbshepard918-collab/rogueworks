...
|| `biomes.json` | **3** | exactly 3 | catacombs / ember_warrens / drowned_vaults. Ambient values are locked-palette colours: `stone_light` #575070, `ember` #a83c1c, `water` #1f5f80. Fog 0.55 / 0.35 / 0.75 (all distinct). Drowned Vaults carries `music: null` — the only null in the file. Each biome carries a P1.2 `modifier`: catacombs_darkness / ember_heat / drowned_water. |
