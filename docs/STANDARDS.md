...
- **Regression assertions must actually test what they claim; a stair check that never calls BFS is not a stair check; dead code that passes is worse than no check (false confidence)** (P1, added 2026-09-14 03:05 after a forge) — tools/qa/regression.py:136 _bfs_reachable defined; lines 159-198 check_stairs never imports or calls it; line 196 PASS emitted with only summary fields — no walls set, no BFS, no target stairs tile

- **A check that SKIPs because the data schema does not support it must not be reported as PASS/verified; the claim ledger must match the tool's actual output** (P2, added 2026-09-14 03:05 after a forge) — tools/qa/regression.py:243 candidates=(damage,armor,crit,power,defense,speed,heal) checked as top-level e.get(f) — items.json effect keys are damage/crit inside effect dict, no top-level combat fields; balance check returns SKIP; fix_balance.py:24 NUMERIC=(damage,armor,crit,value) same mismatch, '500 violations fixed' unverifiable

- **A check that SKIPs because the data schema does not support it must not be reported as PASS/verified; the claim ledger must match the tool's actual output — regression balance must be fixed at the data level, not re-skipped** (P2, added 2026-09-14 06:11 after a forge) — tools.qa.regression --seeds 0 1 2 --turns 300 | grep 'X balance' | wc -l: 26 lines; tier 1: rusty_blade>rusty_spear (3/4/0/14 vs 3/0/0/15), sea_claymore>chipped_hatchet (4/2/0/16 vs 4/0/0/18)

- **Never leave test fixtures in runs/ or assets/ — write them under LOCALAPPDATA/Temp and delete them, or make the tool clean up after itself** (P2, added 2026-09-14 14:05 after a forge) — game/main.py writes `runs/playtest-<seed>.json` by default; after a round completes, all `runs/playtest-*.json` and `runs/selftest-*.json` must match HEAD exactly (no uncommitted modifications). Any verification run output must be written under `%LOCALAPPDATA%/Temp` and the `runs/` copies restored from HEAD before committing.

- **Report artifacts must be committed before BUILD report; metrics may not be retroactively edited to green** (added 2026-09-14 14:05 after a forge) — a PLAYTEST artifact's metrics on disk must equal HEAD; a builder may not `git stash` and replay metrics against a report's claims. SLAP #98: r5 report claimed 'No test fixtures left in runs/' while runs/playtest-0..7.json and runs/selftest-0.json carried metrics different from HEAD. Fix: restore from HEAD and re-run cleanly.

- **Each round must PREPEND a dated entry to docs/PROGRESS.md, write runs/reports/BUILD-<date>.md, tick ROADMAP.md + TICKETS.md. An undocumented round did not happen (STANDARDS P2).** (P2, added 2026-09-14 15:25 after a forge) — ROADMAP.md line 261: '- [ ] P5.6 Modding-lite' still unchecked; docs/PROGRESS.md: zero P5.6 entries; TICKETS.md line 80: P5.6 in open section; no BUILD-2026-09-14 or BUILD-2026-09-15 report exists; git log shows no commit for P5.6 changes; round-r1-forge.log summary admits 'No commit yet — all changes are in working tree'

- **Round protocol (ROADMAP.md §Round protocol item 4): each round must PREPEND a dated entry to docs/PROGRESS.md, write runs/reports/BUILD-<date>.md, and tick the item in docs/ROADMAP.md + docs/TICKETS.md. An undocumented round did not happen.** (P2, added 2026-09-14 16:31 after a forge) — docs/PROGRESS.md: no entry newer than 2026-09-17 (P0.6 Orphan Art Fix); runs/reports/: no BUILD-2026-09-18.md; round-r2-forge.log summary: 'Remaining (not done — session hit tool limit): 1. docs/ROADMAP.md — tick P5.2, 2. docs/PROGRESS.md — prepend dated entry, 3. runs/reports/BUILD-2026-09-18.md — write BUILD report, 4. git add -A && git commit'

- **Docs must accurately describe what happened. Date claims must match the system date and file timestamps.** (P3, added 2026-09-14 16:31 after a forge) — docs/TICKETS.md line 76: DONE 2026-09-18; round-r2-forge.log timestamps show 2026-09-14; system date 2026-09-14 per STANDARDS.md entries

- **Never leave test fixtures in runs/ or assets/ or project root — write them under C:\Users\dbshe\AppData\Local\Temp and delete them, or make the tool clean up after itself (STANDARDS P2).** (P2, added 2026-09-14 16:31 after a forge) — ls /c/Users/dbshe/rogueworks/tmp/: p52-r2rec.json p52-r2rpl.json p52-r2rpl2.json r.json r.json.log r2.json.log — all dated during round 2, none referenced by any project file

- **Report artifacts must be committed before BUILD report; metrics may not be retroactively edited to green. A PLAYTEST artifact's metrics on disk must equal HEAD; a builder may not Saved working directory and index state WIP on master: ea434f6 r42: SLAP #102 CLOSED CLEAN — boss_detail.py fixture absent, all gates green and replay metrics against a report's claims (STANDARDS, added 2026-09-14).** (P1, added 2026-09-14 17:27 after a forge) — git diff HEAD -- runs/playtest-0.json shows fps_equiv 275→320, ms_per_tick 3.634→3.128; selftest-0.json fps_equiv 260→279, tick_ms 0.13→0.20; 8 playtest + 1 selftest JSON modified, all unstaged
