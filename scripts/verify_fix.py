...
pygame.image.save(surface, 'runs/shots/verify_fix.png')
...
for ty in range(0, 22):
    for tx in range(0, 40):
        ...
        if unique > 12:
            text_count += 1
            if text_count <= 5:
                print(f"  Detail at ({tx},{ty}): {unique} colors")

print(f"High-detail tiles (text?): {text_count}")
print(f"Player: sprite={game.world.player.sprite}")
